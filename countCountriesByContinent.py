...
with GraknClient(uri="localhost:48555") as client:
    with client.session(keyspace = "globe") as session:
        with session.transaction().read() as transaction:
            query = [
                'match $country isa country, ' +
                ' has continent "North America", has indepyear $iy; $iy > 1950; get;count;'
            ]
            ...
            countries = [ans.number() for ans in iterator]
            print(str(countries[0]) + " countries")
