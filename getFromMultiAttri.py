    ...
    with client.session(keyspace = "globe") as session:
        with session.transaction().read() as transaction:
            query = [
                ...
                ' has indepyear $iy, has population $p, has surfacearea $sc;' +
                ' $iy > 1980; $sc > 10000; $p < 1000000; get $cname, $p, $sc;'
            ]
            ...
            countries = [[ans.get("cname"), ans.get("p"), ans.get("sc")] for ans in iterator]
            print(countries)
            # result = [ answer.id for answer in answers ]

            for country in countries:
                print(str(country[0].value()) + ": " + 
                        str(country[1].value()/country[2].value()))
